// this was stolen from https://www.lemoda.net/c/write-png/
...
int saveImg(int Nx, int Ny, int ***S, int index, int t) {
  ...
  for (y = 0; y < image.height; y++) {
    for (x = 0; x < image.width; x++) {
      ...
      pixel->red = pix((S[t][x][y] + 1) * 127, image.width);
      pixel->green = pix((S[t][x][y] + 1) * 127, image.height);
    }
  }

  char fileName[100];
  char strNumber[20];
  strcpy(fileName, "output/ising_");
